$("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			type:"GET",//请求类型
			url:$("#basePath").val()+"/dev/appcategory/categorylevellist/"+queryCategoryLevel1,//请求的url
...
$("#queryCategoryLevel2").change(function(){
	...
	if(queryCategoryLevel2 != '' && queryCategoryLevel2 != null){
		$.ajax({
			type:"GET",//请求类型
			url:$("#basePath").val()+"/dev/appcategory/categorylevellist/"+queryCategoryLevel2,//请求的url
...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href=$("#basePath").val()+"/dev/appversion/appversionadd/"+obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href=$("#basePath").val()+"/dev/appversion/appversionmodify?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href=$("#basePath").val()+"/dev/app/appinfomodify/"+ obj.attr("appinfoid");
	}else{
...
var saleSwitchAjax = function(appId,obj){
	$.ajax({
		type:"PUT",
		url:$("#basePath").val()+"/dev/app/sale/"+appId,
		dataType:"json",
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	window.location.href=$("#basePath").val()+"/dev/app/appview/"+ obj.attr("appinfoid");
});
...
$(".deleteApp").on("click",function(){
	...
	if(confirm("你确定要删除APP应用【"+obj.attr("appsoftwarename")+"】及其所有的版本吗？")){
		$.ajax({
			type:"GET",
			url:$("#basePath").val()+"/dev/app/delapp/"+obj.attr("appinfoid"),
